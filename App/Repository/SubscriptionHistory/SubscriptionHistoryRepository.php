...
namespace App\Repository\SubscriptionHistory;
...
class SubscriptionHistoryRepository implements ISubscriptionHistoryRepository {
	...
	private string $table = "subscription_history";
	...
	public function findByOldPlanId(int $id) {
		if(!$registers = $this->database->select("*", $this->table, "old_plan_id = $id")) return $registers;

		return $this->getObject($registers);
	}

	public function findByNewPlanId(int $id) {
		if(!$registers = $this->database->select("*", $this->table, "new_plan_id = $id")) return $registers;
	...
	private function getObject(array $data){
		foreach ($data as $v){
			$this->registers[$v[$this->columns[0]]] = new SubscriptionHistory(
...
interface ISubscriptionHistoryRepository {

	/**
	 * Summary of findBySubscriptionId
	 * @param int $id
	 * @return array|SubscriptionHistory
	 */
	public function findBySubscriptionId(int $id);

	/**
	 * Summary of findByOldPlanId
	 * @param int $id
	 * @return array|SubscriptionHistory
	 */
	public function findByOldPlanId(int $id);
