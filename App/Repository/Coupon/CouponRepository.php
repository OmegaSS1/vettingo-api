...
class CouponRepository implements ICouponRepository {
	...
	public function findByStripeCouponId(int $id) {
		if(!$registers = $this->database->select("*", $this->table, "stripe_coupon_id = $id")) return $registers;
	...
	private function getObject(array $data){
		foreach ($data as $v){
			$this->registers[$v[$this->columns[0]]] = new Coupon(
...
interface ICouponRepository {
	...
	/**
	 * Summary of findByStripeCouponId
	 * @param int $id
	 * @return array|Coupon
	 */
	public function findByStripeCouponId(int $id);
