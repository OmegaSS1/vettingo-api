...
class UpdatePet extends PetAction {
    ...
    protected function action(): Response {
        ...
        try {
            ...
            $this->loggerInterface->info("Falha ao tentar atualizar o pet", ["message" => $e->getMessage(), "code" => $e->getCode(), "line" => $e->getLine(), "file" => $e->getFile()]);
            throw new Exception('Falha ao tentar atualizar o pet', 400);
        }
    ...
    private function validate(array &$form){
        ...
        if($form["hasPedigree"] === true){
            ...
            $pedigree = $this->iPetRepository->findByPedigreeNumber($form["pedigreeNumber"]);
            
            if($pedigree->getId() != $pet->getId()){
